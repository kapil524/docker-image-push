pipeline {
    ...
    environment {
        ...
        AWS_CRED = 'aws-cred-id'          // Jenkins AWS credential ID
        ...
        KEY_NAME = "jenkins-ec2-key"     // The EC2 key pair name in AWS
    }
    stages {
        ...
        stage('Create EC2 & Deploy Container') {
            steps {
                script {
                    ...
                    sh """
                    ssh -o StrictHostKeyChecking=no -i /var/lib/jenkins/.ssh/$KEY_NAME ubuntu@${PUBLIC_IP} \\
